	...
	return class CompositionTracker extends Constructor<PossibleCustomElement, PossibleCustomElementConstructor & T>(
		...
		override attachShadow(options: ShadowRootInit): ShadowRoot {
			...
			for (const child of children) {
				...
				// FIXME type narrowing not working, child is instanceof WithCompositionCallbacks
		...
		get _shadowRootChildren(): CompositionTracker[] {
			...
			// TODO filtering should be done by subclass
		...
		__handleDistributedChildren(slot: HTMLSlotElement) {
			...
			for (let l = added.length, i = 0; i < l; i += 1) {
				...
				// TODO filtering should be specified by subclasses
		...
		traverseComposed(visitor: (node: CompositionTracker) => void, waitForUpgrade = false): Promise<void> | void {
			if (!waitForUpgrade) {
...
// TODO does not belong here, WithCompositionCallbacks should have no idea about Node.
...
// TODO does not belong here, WithCompositionCallbacks should have no idea about Scene.
